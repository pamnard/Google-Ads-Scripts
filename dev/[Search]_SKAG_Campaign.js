function main() {
    ...
    while (campaignPerfomaceRowsIter.hasNext()) {
        ...
        if (CampaignRow) {
            ...
            var negativesListFromCampaign = getCampaignNegatives(); // Получаем минус-слова кампании
            ...
            while (QueryPerfomanceRowsIter.hasNext()) {
                ...
                if (QueryRow) {
                    if (NewKeyword.indexOf(AdGroupName) != -1) {
                        ...
                        var diffWords = queryWords.diff(keywordWords); // Определяем отличающиеся слова
